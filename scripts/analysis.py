...
def main():
    ...
    with torch.no_grad():
        for i, frames in enumerate(tqdm(loader, desc="Collecting Analysis Data")):
            ...
            # Since forward returns (rec, p_y), we need to access internal y.
            # We'll use the model components directly for analysis.
            ...
            for b in range(target.shape[0]):
                stats.append({
                    ...
                    "raw_bytes": raw_bytes,
                    # "compressed_bytes": math.ceil((-torch.sum(torch.log2(p_y[b] + 1e-6)).item()) / 8),
                    "compressed_bytes": total_bytes,
                })

            # ── Latent Collection ───────────────────────────────────────────
            # Sample random spatial locations from the y tensor (B, 128, H', W')
            # y shape: (B, 128, H/8, W/8)
            y_flat = y.permute(0, 2, 3, 1).reshape(-1, 128).cpu().numpy()
    ...
    plt.title(f"Latent Space PCA (128 -> 2D)\nExplained Variance: {sum(pca.explained_variance_ratio_):.2%}", fontsize=14)
    plt.xlabel("Principal Component 1")
    plt.ylabel("Principal Component 2")
    plt.grid(True, linestyle="--", alpha=0.3)

    out_pca = os.path.join(args.run, "latent_space_pca.png")
    plt.savefig(out_pca, dpi=150, bbox_inches="tight")
    print(f"Latent Visualization saved to: {out_pca}")
