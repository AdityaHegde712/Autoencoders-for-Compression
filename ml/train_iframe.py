...
from ml.models.iframe_encoder import AsymmetricAutoencoder
...
def _compute_distortion_loss(
    ...
    distortion = alpha * ssim_loss + (1.0 - alpha) * l1_loss
    return distortion, ssim_loss, l1_loss
...
def compute_loss(
    ...
    """
    distortion, ssim_loss, l1_loss = _compute_distortion_loss(output, target, ssim_module, alpha)
    bpp        = _compute_bpp_loss(likelihoods, target)
    total      = distortion + lambda_rate * bpp
    return total, distortion, bpp, ssim_loss, l1_loss
...
def train():
    ...
    csv_writer.writerow([
        "epoch",
        "train_loss", "train_dist_loss", "train_bpp", "train_ssim_loss", "train_l1_loss",
        "val_loss",   "val_dist_loss",   "val_bpp",   "val_ssim_loss",   "val_l1_loss",
    ])
    ...
    for epoch in range(EPOCHS):
        ...
        for i, frames in enumerate(train_bar):
            ...
            loss, dist_loss, bpp_loss, ssim_loss, l1_loss = compute_loss(
        ...
        with torch.no_grad():
            for i, frames in enumerate(val_bar):
                ...
                v_loss, dist_v, bpp_v, ssim_v, l1_v = compute_loss(
                    x_hat, x, p_y, loss_module, LAMBDA_BITRATE
                )
                ...
                val_bpp  += bpp_v.item()
                val_ssim += ssim_v.item()
                val_l1   += l1_v.item()

                val_bar.set_postfix({
                    "val_dist": f"{dist_v.item():.4f}",
                    "val_ssim": f"{ssim_v.item():.4f}",
                    "val_l1": f"{l1_v.item():.4f}",
                })
        ...
        csv_writer.writerow([
            ...
            f"{train_ssim / n_train:.6f}",
            f"{train_l1   / n_train:.6f}",
            f"{avg_val_loss:.6f}",
            f"{val_dist   / n_val:.6f}",
            f"{val_bpp    / n_val:.6f}",
            f"{val_ssim   / n_val:.6f}",
        ...
        print(f"Train SSIM Loss: {train_ssim / n_train:.4f} | Train L1 Loss: {train_l1 / n_train:.4f}")
        print(f"Validation Loss: {avg_val_loss:.4f}  |  Best: {best_val_loss:.4f}  |  Patience: {patience_counter}/{PATIENCE}")
        print(f"Val SSIM Loss: {val_ssim / n_val:.4f} | Val L1 Loss: {val_l1 / n_val:.4f}")
        print("\n")
